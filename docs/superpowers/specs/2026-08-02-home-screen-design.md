...
6. `GameManager` 根据该预设的完整 `starter_cards` 创建新的运行时玩家和卡牌实例，再初始化棋盘、手牌和事件；
...
| `starter_cards` | 必填数组；完整的固定起始牌组，必须包含且只包含一张牌根；普通卡可重复。 |
...
- 牌根由 `StartingDeckData` 从 `starter_cards` 解析得到，作为首页牌根预览和本局战斗服务选择的唯一来源；
...
首页只传递所选套组。牌根决定的全局规则和不同 `CombatService` 的选择仍属于本局玩法初始化：`GameManager` 在从选中预设的 `starter_cards` 解析牌根后，创建默认或该牌根专用的战斗服务。
...
右侧展示当前已选预设的名称、玩法标签、牌根效果、套组说明、完整起始牌总数和除牌根外的其余起始卡预览。牌根已在上方主预览中展示，不在下方重复渲染。
...
1. 每次刷新预览时，从 `starter_cards` 解析牌根，并为牌根和其余要展示的 `CardData` 创建临时 `CardInstance`；
...
| 层级 | 用例输入 | 预期步骤与结果 |
|---|---|---|
| Resource 单元测试 | 有效复苏套组 | 读取预设 → 校验通过 → `starter_cards` 含且仅含一张 `ROOT` → 所有起始卡有效 → 起始总数为 `starter_cards.size()`。 |
| Resource 单元测试 | 起始牌组没有牌根、含多张牌根、含 `null` | 读取预设 → 校验失败 → 返回具体错误信息 → 预设不成为可开始候选。 |
...
| 启动流程集成测试 | 选择复苏套组后点击开始 | 选择预设 → 仅发射一次 `start_requested(preset)` → `Main` 实例化 `GameManager` 并注入同一预设 → `GameManager` 按 `starter_cards` 创建运行时实例且不额外添加牌根 → 正常进入探索状态。 |
