...
```text
scripts/combatv2/             # 旧版战斗系统，保留现有遭遇入口和旧规则
scripts/combat_framework/     # 新版批次驱动战斗框架
├── protocol/                 # 战斗协议
├── state/                    # 正式状态、快照、草稿与写入器
├── batch/                    # 批次队列和唯一状态提交处理器
├── effects/                  # 标准效果、处理器和状态规则
└── runtime/                  # 战斗驱动、流程提供器、时钟和顶层会话
```

`combat_framework` 不依赖旧版 `CombatService` 推进战斗；旧版入口也不会自动创建新版会话。两套系统的正式路由切换留到后续阶段处理。
