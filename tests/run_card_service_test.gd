...
func _test_starting_deck_cards_are_owned_and_added_to_hand() -> void:
	...
	_expect(service.get_instances().size() == RevivalDeck.starter_cards.size(), "runtime card service tracks every starter instance")
	_expect(service.get_entities().size() == RevivalDeck.starter_cards.size(), "runtime card service tracks every starter entity")

	for index in range(RevivalDeck.starter_cards.size()):
		var entity: CardEntity = service.get_entities()[index]
		_expect(service.get_instances()[index].card_data == RevivalDeck.starter_cards[index], "runtime card service preserves starter card order at index %d" % index)
		_expect(entity in service.hand_area.cards, "runtime card service places starter card %d in hand" % index)
		_expect(entity.drag_layer == service.drag_layer, "runtime card service assigns drag layer to starter card %d" % index)
...
func _test_granted_card_is_tracked_only_after_it_enters_hand() -> void:
	...
	_expect(not service.grant_to_hand(card_data), "runtime card service rejects reward card when hand is full")
	...
	_expect(service.grant_to_hand(card_data), "runtime card service grants card when hand has space")
	_expect(service.get_instances().size() == 1, "successful reward grant tracks its instance")
	_expect(service.get_instances()[0].cur_zone == CardInstance.ZONE.HAND, "successful reward grant marks its instance as in hand")
	_expect(service.get_entities()[0] in service.hand_area.cards, "successful reward grant adds its entity to hand")
...
func _test_existing_card_can_return_to_a_full_hand_without_duplication() -> void:
	...
	if service == null:
		return
	_expect(service.initialize_starting_deck(RevivalDeck), "fixture starter deck initializes before guide return")
	...
	_expect(service.return_existing_to_hand(guide, true), "runtime card service returns an existing guide card despite a full hand")
	_expect(guide in service.hand_area.cards, "returned guide card enters hand")
	_expect(service.hand_area.max_hand_size >= service.hand_area.cards.size(), "guide return expands hand capacity only as needed")
	_expect(service.get_entities().size() == existing_count, "returning an untracked guide card does not duplicate player ownership")
	_expect(service.return_existing_to_hand(guide, true) == false, "runtime card service rejects returning a card already in hand")
...
func _test_existing_card_can_return_with_temporary_overflow() -> void:
	...
	if service == null:
		return
	_expect(service.initialize_starting_deck(RevivalDeck), "fixture starter deck initializes before temporary return")
	var returned_card: CardEntity = service.get_entities()[0]
	_expect(service.hand_area.remove_card(returned_card), "fixture removes the owned tail card from hand before retreat")
	service.hand_area.max_hand_size = service.hand_area.cards.size()
	...
	_expect(returned_card in service.get_entities(), "temporary return preserves existing entity ownership")
	_expect(returned_card.card_instance in service.get_instances(), "temporary return preserves existing instance ownership")
...
func _test_reclaimed_card_is_forgotten_without_being_freed() -> void:
	...
	if service == null:
		return
	_expect(service.initialize_starting_deck(RevivalDeck), "fixture starter deck initializes before market reclaim")
	...
	_expect(card not in service.get_entities(), "forgotten reclaimed card no longer has entity ownership")
	_expect(card.card_instance not in service.get_instances(), "forgotten reclaimed card no longer has instance ownership")
	_expect(is_instance_valid(card), "runtime card service leaves reclaimed entity alive for DragLayer cleanup")
...
func _create_service():
	...
	if service_script == null:
		_expect(false, "runtime card service script exists so GameManager can delegate card lifecycle ownership")
		return null
	...
	_expect(service.configure(card_manager, hand, drag_layer), "runtime card service configures real card, hand, and drag dependencies")
	return service
