...
func _test_boss_victory_reuses_reward_pipeline_before_removal() -> void:
	...
	_expect(
		...
		"boss victory delegates event removal through exploration"
	)
...
func _configure_coordinator(coordinator, fixture: Dictionary) -> bool:
	_expect(
		_configure_argument_count(coordinator) == 7,
		"encounter resolution accepts PlayerData and reward RNG dependencies"
	)
	if _configure_argument_count(coordinator) != 7:
		return false
...
func _create_fixture(event_type: EventData.EventType) -> Dictionary:
	...
	_expect(
		exploration.configure(EventLib.new(), board, exploration_config),
		"fixture configures exploration facade"
	)
