...
func play_trigger(_clip: CombatPresentationClip, duration: float) -> CombatAnimationHandle:
	...
	tween.tween_property(visual_root, ^"scale", original_scale * 1.06, half_duration)
	tween.tween_property(visual_root, ^"scale", original_scale, half_duration)
	return handle
...
func animate_points_change(
	...
	return _animate_number(points_label, clip, duration)
