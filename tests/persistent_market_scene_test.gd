...
func _run_tests() -> void:
	...
	_expect((market.get_node("HeaderRow/TitleLabel") as Label).get_theme_font_size("font_size") == 19, "market title is readable")
	_expect((market.get_node("HeaderRow/RefreshButton") as Button).get_theme_font_size("font_size") == 14, "refresh button text is readable")
	_expect((market.get_node("HeaderRow/RefreshCostLabel") as Label).get_theme_font_size("font_size") == 13, "refresh cost is readable")
	_expect((market.get_node("OfferRow/OfferSlot1/PriceLabel") as Label).get_theme_font_size("font_size") == 15, "offer price is readable")
	_expect((market.get_node("ReclaimArea/TitleLabel") as Label).get_theme_font_size("font_size") == 16, "reclaim title is readable")
	_expect((market.get_node("ReclaimArea/HintLabel") as Label).get_theme_font_size("font_size") == 13, "reclaim hint is readable")
	_expect((market.get_node("HeaderRow/TitleLabel") as Label).get_theme_color("font_color").get_luminance() > 0.65, "market title has strong contrast")
	_expect((market.get_node("ReclaimArea/HintLabel") as Label).get_theme_color("font_color").get_luminance() > 0.35, "reclaim hint has readable contrast")
