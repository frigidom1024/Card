...
@export_group("Layout")
@export_range(270.0, 800.0, 1.0) var plaque_width := 584.0:
	set(value):
		plaque_width = value
		_apply_visuals()
...
@export var plaque_color := Color("07101bef"):
	set(value):
		plaque_color = value
		_apply_visuals()
...
@export var trim_color := Color("b7964fbf"):
	set(value):
		trim_color = value
		_apply_visuals()
@export var text_color := Color("f4ead2"):
	set(value):
		text_color = value
		_apply_visuals()
@export var muted_text_color := Color("d1c5a9"):
	set(value):
...
func _apply_visuals() -> void:
	...
	if identity != null:
		identity.position = Vector2(58, 16)
	...
	if subtitle != null:
		subtitle.position = Vector2(58, 46)
		subtitle.size = Vector2(plaque_width - 82, 22)
	...
	if map_label != null:
		map_label.position = Vector2(24, 72)
		map_label.size = Vector2(plaque_width - 48, 22)
	...
	if divider != null:
		divider.position = Vector2(24, 101)
	...
	if vitality_title != null:
		vitality_title.position = Vector2(24, 112)
		vitality_title.size = Vector2(120, 20)
	...
	if vitality_bar != null:
		...
		vitality_bar.size = Vector2(plaque_width - 48, 11)
	...
	if faith_seal != null:
		...
		faith_seal.size = Vector2(262, 30)
		faith_seal.add_theme_stylebox_override("panel", _make_panel_style(Color("172333e8"), 5, Color("75809a"), 1))

	var faith_value := get_node_or_null("FaithSeal/FaithValue") as Label
	if faith_value != null:
		faith_value.position = Vector2.ZERO
		faith_value.size = faith_seal.size if faith_seal != null else Vector2(262, 30)
		faith_value.add_theme_color_override("font_color", text_color)

	var gold_seal := get_node_or_null("GoldSeal") as Panel
	if gold_seal != null:
		gold_seal.position = Vector2(298, 194)
		gold_seal.size = Vector2(262, 30)
		gold_seal.add_theme_stylebox_override("panel", _make_panel_style(Color("292b27e8"), 5, Color("8d8065"), 1))

	var gold_value := get_node_or_null("GoldSeal/GoldValue") as Label
	if gold_value != null:
		gold_value.position = Vector2.ZERO
		gold_value.size = gold_seal.size if gold_seal != null else Vector2(262, 30)
		gold_value.add_theme_color_override("font_color", text_color)
	...
	if status_label != null:
		...
		status_label.add_theme_color_override("font_color", Color("f0d8c0"))
