...
func _test_display_only_card_ignores_gameplay_interactions() -> void:
	...
	_expect(panel != null and panel.visible, "interactive card can show card-owned hover details")
	...
	_expect(panel != null and not panel.visible, "display-only mode clears its card-owned hover details")
