...
func _test_monster_victory_resolves_event_and_unlocks_exploration() -> void:
	...
	_expect(not event_node.event_instance.is_resolved, "victory stays unresolved until settlement confirmation")
	...
	_expect(manager.drag_layer.is_interaction_locked(), "victory keeps exploration locked before confirmation")
...
func _test_defeat_emits_failure_and_keeps_exploration_locked() -> void:
	...
	_expect(not event_node.event_instance.is_resolved, "defeat does not resolve the encounter event before confirmation")
	...
	_expect(manager.drag_layer.is_interaction_locked(), "defeat keeps exploration locked before confirmation")

	_confirm_combat_settlement(manager)
	_expect(not event_node.event_instance.is_resolved, "confirmed defeat does not resolve the encounter event")
...
func _test_shop_event_routes_purchase_and_close() -> void:
	...
	var event_node := _attach_event(manager.board, EventData.EventType.SHOP, Vector2i(1, 0), shop_content)
	var hand_before: int = manager.hand_area.cards.size()

	_expect(_place_card(manager, _horizontal_card_center(manager.board, Vector2i(1, 0)), 90.0, CardData.CardType.ROOT, 0) != null, "shop setup places root over shop")
...
func _test_treasure_event_routes_claim_and_resolves() -> void:
	...
	var event_node := _attach_event(manager.board, EventData.EventType.TREASURE, Vector2i(1, 0), treasure_content)
	var hand_before: int = manager.hand_area.cards.size()

	_expect(_place_card(manager, _horizontal_card_center(manager.board, Vector2i(1, 0)), 90.0, CardData.CardType.ROOT, 0) != null, "treasure setup places root over treasure")
	var cards_before: int = manager.cards_inst.size()
	await process_frame
	...
	_expect((event_node.event_instance.runtime_state as TreasureRuntimeState).options.size() == 3, "treasure creates two card rewards and one gold reward")
	...
	_expect(not event_node.event_instance.is_resolved, "closing treasure does not discard the unresolved reward")
	_expect(manager.drag_layer.is_interaction_locked(), "closing treasure keeps exploration locked until a reward is claimed")

	var claim_button := treasure_view.find_child("OfferSlot1", true, false).find_child("ActionButton", true, false) as Button
	...
	_expect(manager.cards_inst.size() == cards_before + 1, "treasure card reward grants a persistent card instance")
	_expect(manager.hand_area.cards.size() == hand_before + 1, "treasure card reward adds the card to hand")
