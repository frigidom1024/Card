...
const NextCardPointBonusRule = preload(
...
func _test_starter_card_point_contract() -> void:
	...
	_expect(Blade.effect_rules.size() == 1, "rib blade has one placement rule")
	if Blade.effect_rules.size() == 1:
		_expect(
			Blade.effect_rules[0].get_script() == NextCardPointBonusRule
			and Blade.effect_rules[0].get("bonus_points") == 1,
			"rib blade grants the next card one point"
		)
...
func _test_full_ribwood_card_pool_contract() -> void:
	...
	_expect_card(Blade, 2, 0, 4, CardData.Rarity.COMMON, "从灰白肋骨上削下的短刃。每当你在本牌前方放置一张卡牌时，该卡牌获得 +1 点数。")
	_expect_card(Tinder, 1, 0, 3, CardData.Rarity.COMMON, "从旧营火留下的干燥火绒。无额外效果。")
	_expect_card(Shield, 1, 1, 4, CardData.Rarity.COMMON, "可以折起挂在行囊侧面的肋骨圆盾。每当你在本牌前方放置一张卡牌时，该卡牌获得 +1 护甲。")
	_expect_card(Flask, 1, 1, 4, CardData.Rarity.COMMON, "保温的骨髓水囊。无额外效果。")
	_expect_card(RibNail, 3, 0, 8, CardData.Rarity.COMMON, "从肋骨边缘拔下的长钉，沉重却可靠。无额外效果。")
	_expect_card(ShieldFragment, 1, 1, 8, CardData.Rarity.COMMON, "裂开的肋盾碎片。本牌前方的下一张卡牌获得 +2 护甲。此效果仅生效 1 次。")
	_expect_card(AmberMarrowBottle, 2, 2, 8, CardData.Rarity.COMMON, "封着暖金色骨髓的玻璃瓶。无额外效果。")
	_expect_card(BoneArmorRoundShield, 1, 2, 8, CardData.Rarity.COMMON, "用旧骨甲削成的圆盾。本牌前方的下一张卡牌获得 +2 护甲。此效果仅生效 2 次。")
	_expect_card(EmberBlade, 3, 0, 10, CardData.Rarity.RARE, "仍带余温的短刃。只要本牌仍在牌链中，当你放置的卡牌使牌链达到 4 张或以上时，该头部卡牌获得 +2 点数。此效果仅生效 1 次。")
	_expect_card(WarmthCharm, 2, 1, 10, CardData.Rarity.RARE, "缠在腕上的余温护符。只要本牌仍在牌链中，当你放置的卡牌使牌链达到 4 张或以上时，该头部卡牌获得 +2 护甲。此效果仅生效 1 次。")
	_expect_card(ScarBottle, 2, 2, 8, CardData.Rarity.COMMON, "装着结痂粉末的小瓶。无额外效果。")
	_expect_card(GraveBeetle, 3, 2, 14, CardData.Rarity.RARE, "守墓甲虫伏在行囊上，硬壳会替旅人承受撞击。无额外效果。")
	_expect_card(BoneStitchNeedle, 2, 0, 12, CardData.Rarity.RARE, "缝补披风和行囊的骨针。当你在本牌前方放置一张具有护甲的卡牌时，该卡牌获得 +2 点数。此效果仅生效 2 次。")
	_expect_card(OldChestCloak, 2, 2, 10, CardData.Rarity.COMMON, "胸前磨白的旧斗篷，缝线里夹着干燥草药。无额外效果。")
	_expect_card(LastStandStrike, 4, 0, 12, CardData.Rarity.RARE, "走投无路时挥出的重击。无额外效果。")
	_expect_card(WhiteHornRelic, 3, 1, 16, CardData.Rarity.EPIC, "白角守墓鹿遗下的一截角片，既锋利又能挡住一次冲撞。无额外效果。")
	_expect_card(SternumPlate, 2, 2, 16, CardData.Rarity.EPIC, "厚重的胸骨护板。本牌前方的下一张卡牌获得 +2 护甲。此效果仅生效 1 次。")
	_expect_card(GuardianEmber, 2, 3, 16, CardData.Rarity.EPIC, "守护余烬藏在灯罩里，光不强，却足以护住下一步。无额外效果。")

	_expect_rule(Blade, NextCardPointBonusRule, {"bonus_points": 1, "effective_count": -1})
